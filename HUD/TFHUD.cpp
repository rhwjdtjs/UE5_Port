...
void ATFHUD::DrawHUD()
{
	...
	if (GEngine) {
		...
		if (HUDPackage.CrosshairsCenter) {
			...
			DrawCrosshair(HUDPackage.CrosshairsCenter, ViewportCenter,Spread); //중앙 크로스헤어 그리기
		}
		if (HUDPackage.CrosshairsLeft) {
			FVector2D Spread(-SpreadScaled, 0.f); //왼쪽 크로스헤어 스프레드 계산
			DrawCrosshair(HUDPackage.CrosshairsLeft, ViewportCenter, Spread); //중앙 크로스헤어 그리기
		}
		if (HUDPackage.CrosshairsRight) {
			FVector2D Spread(SpreadScaled, 0.f); //오른쪽 크로스헤어 스프레드 계산
			DrawCrosshair(HUDPackage.CrosshairsRight, ViewportCenter, Spread); //중앙 크로스헤어 그리기
		}
		if (HUDPackage.CrosshairsTop) {
			FVector2D Spread(0.f, -SpreadScaled); //위쪽 크로스헤어 스프레드 계산
			DrawCrosshair(HUDPackage.CrosshairsTop, ViewportCenter, Spread); //중앙 크로스헤어 그리기
		}
		if (HUDPackage.CrosshairsBottom) {
			FVector2D Spread(0.f, SpreadScaled); //아래쪽 크로스헤어 스프레드 계산
			DrawCrosshair(HUDPackage.CrosshairsBottom, ViewportCenter,Spread); //중앙 크로스헤어 그리기
		}
	}
}

void ATFHUD::DrawCrosshair(UTexture2D* Texture, FVector2D ViewportCenter, FVector2D Spread)
{
